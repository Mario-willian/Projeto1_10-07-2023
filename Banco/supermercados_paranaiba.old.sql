...
CREATE TABLE IF NOT EXISTS `supermercados_paranaiba`.`recisoes` (
  ...
  `data_inicio_aviso` DATETIME NOT NULL,
  `data_fim_aviso` DATETIME NOT NULL,
...
CREATE TABLE IF NOT EXISTS `supermercados_paranaiba`.`lembretes` (
  ...
  `data_desativada` VARCHAR(45) NOT NULL,
  `status` ENUM('Ativo', 'Inativo') NOT NULL,
  `data_criacao` DATETIME NOT NULL,
...
ALTER TABLE `supermercados_paranaiba`.`funcionarios` 
CHANGE COLUMN `data_nascimento` `data_nascimento` DATE NOT NULL ;
ALTER TABLE `supermercados_paranaiba`.`ferias` 
DROP COLUMN `status`;
